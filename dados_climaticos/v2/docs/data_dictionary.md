...
| `cloud_cover_percent` | **percentual_nuvens** | Float | **O que é:** Percentual da área do município coberta por nuvens no momento da foto do satélite (de **0 a 100%**). Obtido através da banda de Máscara de Nuvens (CLM).<br>**Como ler os dados:**<br>- **`0% a 10%`:** Céu limpo. Os dados de NDVI extraídos neste dia são extremamente confiáveis (geralmente apresentam valores altos e estáveis).<br>- **`Acima de 40% a 50%`:** Muita nebulosidade. Se o NDVI cair bruscamente neste mesmo dia, é altíssima a probabilidade de ser uma ilusão de ótica causada pela sombra das nuvens, e não um desmatamento real. Dias com 100% de nuvens costumam mascarar completamente a vegetação, podendo gerar dados muito distorcidos ou nulos. |
